...
function AllAdminProducts() {
  ...
  const fetchData = async () => {
    const response = await fetch('https://didwaniacreations.in/products/getallproducts');
  ...
  const handleDeleteProduct = async(productId)=>{
    ...
    const response = await fetch(`https://didwaniacreations.in/products/deleteproduct/${productId}`,{
      method: "DELETE"
    })
    const json = await response.json()
          ...
          {currentProducts.map((product) => (
            <Link to={`/product/${product._id}`} className="product-item" key={product._id}>
              {product.productImages && product.productImages.length > 0 ? (
                <img
                  src={`https://didwaniacreations.in/uploads/${product.productImages[0]}`}
                  alt={`https://didwaniacreations.in/uploads/${product.productImages[0]}`}
                  className="hoverable"
                />
              ) : (
