...
useEffect(() => {
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(`https://didwaniacreations.in:5000/products/getproductbyid/${id}`)
      const json = await response.json();
      if(response.ok)
      {
        setCartItems(json)
...
//   }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`https://didwaniacreations.in:5000/bills/billforone/${user.user?._id}/${id}`, {
        method: "POST",
                ...
                <div key={cartItems._id} className="cItem1 cItem11">
                  <div className="cItem cItemm">
                    <div className="cItemImg cItemImgg">
                      <img src={`https://didwaniacreations.in:5000/uploads/${cartItems.product?.productImages[0]}`} alt={cartItems.product?.title} />
                    </div>
