...
useEffect(() => {
  const fetchData = async () => {
    try {
      ...
      const productPromises = adtItems.map(item => 
        fetch(`https://backend.didwaniacreations.in/products/getproductbyid/${item.product}`, {
          headers: {
            'Authorization': `Bearer ${user.token}`,
          }
  ...
  const paymentHandler = async(e) =>{
    ...
    const response = await fetch('https://backend.didwaniacreations.in/order',{
      method: "POST",
      body: JSON.stringify({
        amount,
        currency,
    ...
    var options = {
      ...
      "handler": async function(response){
        ...
        const validateRes = await fetch('https://backend.didwaniacreations.in/order/validate',{
          method: "POST",
        ...
        {
          try{
            const response = await fetch(`https://backend.didwaniacreations.in/bills/billforcart/${user.user?._id}`, {
              method: "POST",
                ...
                <div key={item._id} className="cItem1 cItem11">
                  <div className="cItem cItemm">
                    <div className="cItemImg cItemImgg">
                      <img src={`https://backend.didwaniacreations.in/uploads/${item.productDetails?.product?.productImages[0]}`} alt={item.productDetails?.product?.title} />
                    </div>
