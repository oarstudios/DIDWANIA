...
function Billing() {
  ...
  const fetchData = async () => {
    if (user) {
      const response = await fetch(`https://didwaniacreations.in:5000/users/getuserbyid/${user.user?._id}`, {
        headers: {
...
useEffect(() => {
  const fetchData = async () => {
    try {
      ...
      const productPromises = adtItems.map(item => 
        fetch(`https://didwaniacreations.in:5000/products/getproductbyid/${item.product}`, {
          headers: {
            'Authorization': `Bearer ${user.token}`,
          }
  ...
  const paymentHandler = async(e) =>{
    ...
    const response = await fetch('https://didwaniacreations.in:5000/order',{
      method: "POST",
      body: JSON.stringify({
        amount,
        currency,
    ...
    var options = {
      ...
      "handler": async function(response){
        ...
        const validateRes = await fetch('https://didwaniacreations.in:5000/order/validate',{
          method: "POST",
        ...
        {
          try{
            const response = await fetch(`https://didwaniacreations.in:5000/bills/billforcart/${user.user?._id}`, {
              method: "POST",
                ...
                <div key={item._id} className="cItem1 cItem11">
                  <div className="cItem cItemm">
                    <div className="cItemImg cItemImgg">
                      <img src={`https://didwaniacreations.in:5000/uploads/${item.productDetails?.product?.productImages[0]}`} alt={item.productDetails?.product?.title} />
                    </div>
