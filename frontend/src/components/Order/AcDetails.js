...
function AcDetails() {
  ...
  useEffect(()=>{
    const fetchData = async()=>{
      const response = await fetch(`https://didwaniacreations.in:5000/users/getuserbyid/${id}`)
...
const newUserData = {
  ...
  const handleSubmit = async(e) =>{
    ...
    try{
      ...
      const response = await fetch(`https://didwaniacreations.in:5000/users/updateuser/${id}`, {
        method: 'PUT',
        headers: {
