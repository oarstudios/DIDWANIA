  ...
  if (adtItems && adtItems.length > -1) {
    ...
    const handleRemove = async(id) => {
      ...
      try {
        ...
        const response = await fetch(`https://didwaniacreations.in:5000/users/removefromcart/${user.user?._id}`, {
          method: "DELETE",
                  ...
                  <Link to={`/cartproduct/${item.productDetails?.product?._id}/${item.quantity}`} style={{ textDecoration: "none", cursor: "pointer" }}>
                    <div className="cItem">
                      <div className="cItemImg">
                        {" "}
                        <img src={`https://didwaniacreations.in:5000/uploads/${item.productDetails?.product?.productImages[0]}`} alt={item.title} />{" "}
                      </div>
